DROP TABLE IF EXISTS `borhandw`.dwh_dim_api_error_codes;

CREATE TABLE `borhandw`.`dwh_dim_api_error_codes` (
...
CREATE TRIGGER `borhandw`.`dwh_dim_api_error_code_setcreationtime_oninsert` BEFORE INSERT
    ON `borhandw`.`dwh_dim_api_error_codes`
    FOR EACH ROW 
