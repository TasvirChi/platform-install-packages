...
Refer to the [All-In-One Borhan Server Installation Guide](https://github.com/borhan/platform-install-packages/blob/master/doc/install-borhan-deb-based.md) for more notes about deploying Borhan in deb supported environments.    
...
* For a cluster install, it is very important to use the [debconf response file] (https://github.com/borhan/platform-install-packages/blob/Jupiter-10.16.0/deb/borhan_debconf_response.sh) because otherwise, the MySQL 'borhan' passwd is autogenerated by the installer. This is fine for a standalone server but for a cluster, passwd must be the same on all. 
