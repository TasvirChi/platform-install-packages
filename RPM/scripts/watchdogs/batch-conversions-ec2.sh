#!/bin/bash - 
#===============================================================================
#          FILE: batch-conversions-ec2.sh
#         USAGE: ./batch-conversions-ec2.sh 
#   DESCRIPTION: A simple wd script to monitor batch conversion queue size and 
# 		 lunch more EC2 instances with Chef setting them up a batch nodes. 
#       OPTIONS: ---
#  REQUIREMENTS: ---
#          BUGS: ---
#         NOTES: ---
#        AUTHOR: Jess Portnoy <jess.portnoy@borhan.com>
#  ORGANIZATION: Borhan, inc.
#       CREATED: 05/07/14 08:23:48 EDT
#      REVISION:  ---
#===============================================================================

#set -o nounset                              # Treat unset variables as an error

SYSTEM_INI=/etc/borhan.d/system.ini
if [ -r "$SYSTEM_INI" ];then
        . /etc/borhan.d/system.ini
else
        echo "Could not source $SYSTEM_INI:("
        exit 1
fi
if [ $# -lt 3 ];then
        echo "Usage: $0 <AMI id> <warning threshold> <critical threshold>"
        exit 2
fi
AMI_IMG=$1
LOWER_THRESHOLD=$2
UPPER_THRESHOLD=$3
OUT=`php /opt/borhan/app/plugins/monitor/nagios/exec.php --warning-threshold $LOWER_THRESHOLD --error-threshold $UPPER_THRESHOLD --script "/opt/borhan/app/tests/monitoring/api_v3/getBatchQueueSize.php --service-url $BORHAN_VIRTUAL_HOST_NAME --job-type CONVERT"`
RC=$?
if [ $RC -eq 1 ];then
echo "Reached WARNING threshold"
# here you can do some actions, for example:
# send yourself an email
# launch instances
fi

# we are at CRITICAL, lets launch

if [ $RC -eq 2 ];then
echo "$OUT
Lunching another batch instance ..
"
knife ec2 server create --availability-zone us-east-1d --flavor m3.medium --image $AMI_IMG --identity-file ~/csi.pem --run-list "recipe[nfs],recipe[borhan::batch]" --ssh-user ec2-user 2>&1 | tee /tmp/log
fi


