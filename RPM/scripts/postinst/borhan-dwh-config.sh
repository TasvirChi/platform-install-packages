#!/bin/bash -e 
#===============================================================================
#          FILE: borhan-dwh-config.sh
#         USAGE: ./borhan-dwh-config.sh 
...
if ! rpm -q borhan-dwh;then
	echo -e "${BRIGHT_BLUE}Skipping as borhan-dwh is not installed.${NORMAL}"
	exit 0 
fi
if [ ! -r $BASE_DIR/app/base-config.lock ];then
	`dirname $0`/borhan-base-config.sh "$1"
else
...
TABLES=`echo "show tables" | mysql -h$DWH_HOST -u$SUPER_USER -p$SUPER_USER_PASSWD -P$DWH_PORT borhandw 2> /dev/null`
if [ -z "$TABLES" ];then 
...
cat << EOF
The Borhan DWH DB seems to already be installed.
DB creation will be skipped.
EOF
fi
chown -R $OS_BORHAN_USER $BASE_DIR/pentaho/pdi $BASE_DIR/dwh/logs
...
ln -sf $APP_DIR/configurations/cron/dwh /etc/cron.d/borhan-dwh
# Alas, we only work well with Sun's Java so, first lets find the latest version we have for it [this package is included in Borhan's repo, as taken from Oracle's site
