...
Epoch: 1
Release: 1
Summary: Borhan kRecord - used for recording from web cam
License: AGPLv3+	
URL: http://borhan.org
...
This package installs the Borhan kRecord - used for recording from web cam.
...
* Sat Feb 1 2014 Jess Portnoy <jess.portnoy@borhan.com> - v1.7.0-1
- Publish Live Streams
  Use the following flashvars to publish live streams:
	isLive: "true"
	rtmpHost : address of a server that holds the app that handles publishing live streams (can be retrieved by parsing BorhanLiveStreamEntry.primaryBroadcastingUrl)
	fmsApp: name of the application that will handle live streaming (can be retrieved by parsing BorhanLiveStreamEntry.primaryBroadcastingUrl)
	streamName: name of the published stream (BorhanLiveStreamEntry.streamName)
