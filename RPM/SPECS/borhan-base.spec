...
Source25: borhan_populate.template
Source26: borhan_batch.template
...
This is the base package, needed for any Borhan server role.
...
mkdir -p $RPM_BUILD_ROOT/etc/borhan.d
...
sed -i "s#^;bmc_version = @BMC_VERSION@#bmc_version = %{_bmc_version}#g" $RPM_BUILD_ROOT%{prefix}/app/configurations/local.template.ini
...
sed -i "s#^;bmc_login_version = @BMC_LOGIN_VERSION@#bmc_login_version = %{bmc_login_version}#g" $RPM_BUILD_ROOT%{prefix}/app/configurations/local.template.ini
...
sed -i "s#^;bdp3_wrapper_version = @BDP3_WRAPPER_VERSION@#bdp3_wrapper_version = %{bdp3_wrapper_version}#g" $RPM_BUILD_ROOT%{prefix}/app/configurations/local.template.ini
...
# we have our own that is provided with the borhan-monit package
...
cp %{SOURCE11} $RPM_BUILD_ROOT%{prefix}/app/alpha/crond/borhan/clear_cache.sh
...
# we bring a1nother in borhan-batch
...
cat > $RPM_BUILD_ROOT%{_sysconfdir}/profile.d/borhan_base.sh << EOF
...
cat > $RPM_BUILD_ROOT%{_sysconfdir}/ld.so.conf.d/borhan_base.conf << EOF
%{prefix}/lib
...
usermod -a -G %{borhan_group} %{apache_user}
...
ln -sf %{prefix}/app/configurations/system.ini /etc/borhan.d/system.ini
ln -sf %{prefix}/app/api_v3/web %{prefix}/app/alpha/web/api_v3
chown apache.borhan %{prefix}/web/content/entry %{prefix}/web/content/uploads/  %{prefix}/web/tmp/
...
if [ "$1" = 2 ];then
	if [ -r "%{prefix}/app/configurations/local.ini" -a -r "%{prefix}/app/configurations/base.ini" ];then
		sed -i "s@^\(borhan_version\).*@\1 = %{version}@g" %{prefix}/app/configurations/local.ini
		...
		chown -R %{borhan_user}.%{apache_user} %{prefix}/app/cache/ %{prefix}/log %{prefix}/var/run
		...
		if [ -r /etc/borhan.d/system.ini ];then
			...
			echo "GRANT INSERT,UPDATE,DELETE,SELECT,ALTER,DROP,CREATE ON borhan.* TO '$DB_USER'@'%';FLUSH PRIVILEGES;"|mysql -h$DB1_HOST -u $SUPER_USER -p$SUPER_USER_PASSWD -P$DB1_PORT
		fi
		php %{prefix}/app/deployment/updates/update.php -i -d >> /opt/borhan/log/kalt_up.log 2>&1
		php %{prefix}/app/deployment/updates/update.php -i -s >> /opt/borhan/log/kalt_up.log 2>&1
		php %{prefix}/app/deployment/base/scripts/installPlugins.php >> /opt/borhan/log/kalt_up.log 2>&1
...
%preun
if [ "$1" = 0 ] ; then
	rm -f %{_sysconfdir}/borhan.d/system.ini
	rm -f %{prefix}/app/alpha/web/api_v3
	rm -f %{_sysconfdir}/logrotate.d/borhan_base
	# get rid of stray symlinks.
	find %{_sysconfdir}/httpd/conf.d/ -type l -name "*borhan*" -exec rm {} \;
...
%config %{_sysconfdir}/profile.d/borhan_base.sh
%config %{_sysconfdir}/ld.so.conf.d/borhan_base.conf


%dir /etc/borhan.d
%defattr(-, %{borhan_user}, %{apache_group} , 0775)
%dir %{prefix}/log
%dir %{prefix}/var/run
%dir %{prefix}/tmp
...
%changelog
* Fri Jan 6 2017 jess.portnoy@borhan.com <Jess Portnoy> - 12.8.0-10
...
* Mon Dec 19 2016 jess.portnoy@borhan.com <Jess Portnoy> - 12.7.0-6
...
* Tue Dec 6 2016 jess.portnoy@borhan.com <Jess Portnoy> - 12.6.0-6
...
* Tue Nov 22 2016 jess.portnoy@borhan.com <Jess Portnoy> - 12.5.0-4
...
- SUP-9501 - Query - in the BMC - Entitlements Settings are empty
...
- PLAT-6412 - add BorhanBaseEntryBaseFilter userIdNotIn field
...
* Mon Oct 10 2016 jess.portnoy@borhan.com <Jess Portnoy> - 12.3.0-7
...
* Wed Oct 6 2016 jess.portnoy@borhan.com <Jess Portnoy> - 12.3.0-5
- disbale Default_Akamai_HLS_direct since we want the nginx vod-module profile to be used [ID 1001, system_name: Borhan HLS segmentation]

* Tue Sep 27 2016 jess.portnoy@borhan.com <Jess Portnoy> - 12.3.0-1
- Ver Bounce to 12.3.0

* Fri Sep 23 2016 jess.portnoy@borhan.com <Jess Portnoy> - 12.2.0-5
...
- PLAT-5983 - Sphinx improvement - add partnerId to privacy_by_contexts
- PLAT-5963 - Add makeHidden property to Borhan_Client_Type_LiveEntryRecordingOptions
...
* Mon Sep 12 2016 jess.portnoy@borhan.com <Jess Portnoy> - 12.1.0-8
- PLAT-5105 - playManifest refactoring for live - delivery profile
...
- SUP-8905 - Cross Borhan distribution failing intermittently 
...
* Sun Aug 28 2016 jess.portnoy@borhan.com <Jess Portnoy> - 12.0.0-9
...
* Fri Aug 12 2016 jess.portnoy@borhan.com <Jess Portnoy> - 11.21.0-6
...
* Tue Aug 2 2016 jess.portnoy@borhan.com <Jess Portnoy> - 11.20.0-8
...
* Tue Jul 19 2016 jess.portnoy@borhan.com <Jess Portnoy> - 11.19.0-4
- SUP-8470 - Thumbnail Preview Displayed Direction
- SUP-7683 - API calls failing (Limited to metadata->update)
- PLAT-5725 - Add user-role CAPTURE_DEVICE_ROLE to partner 0
- PLAT-5326 - As a Borhan platform developer, I'd like to set an entry template on an event

* Tue Jul 12 2016 jess.portnoy@borhan.com <Jess Portnoy> - 11.19.0-1
- Ver Bounce to 11.19.0

* Tue Jul 5 2016 jess.portnoy@borhan.com <Jess Portnoy> - 11.18.0-7
...
* Tue Jun 21 2016 jess.portnoy@borhan.com <Jess Portnoy> - 11.17.0-8
- PLAT-5484 - As a Borhan platform developer, I'd like to filter events by linked resource system name
...
* Fri Jun 3 2016 jess.portnoy@borhan.com <Jess Portnoy> - 11.16.0-8
...
* Sun May 8 2016 jess.portnoy@borhan.com <Jess Portnoy> - 11.13.0-8
...
* Mon Apr 25 2016 jess.portnoy@borhan.com <Jess Portnoy> - 11.13.0-6
- PLAT-4961 - Support passthrough as part of Slating Epic
- PLAT-4962 - Support return to defined entry as part of Slating development
...
* Sun Apr 10 2016 jess.portnoy@borhan.com <Jess Portnoy> - 11.12.0-6
...
* Sun Mar 27 2016 jess.portnoy@borhan.com <Jess Portnoy> - 11.11.0-8
...
* Fri Mar 11 2016 jess.portnoy@borhan.com <Jess Portnoy> - 11.10.0-8
- PLAT-5056 - Enable content Pre-positioning on KES - only download
- PLAT-5091 - Expose Exception throwing in BorhanClient so it can be overridden by implementors
...
* Mon Feb 29 2016 jess.portnoy@borhan.com <Jess Portnoy> - 11.9.0-12
...
- PLAT-5126 - BorhanCaptionAssetItemFilter returns BorhanCaptionAsset
...
* Sun Feb 14 2016 jess.portnoy@borhan.com <Jess Portnoy> - 11.8.0-11
...
- PLAT-4839 - Borhan's webex account "borhanwebexhost" records meetings in *.MP4 instead of *.arf
...
* Sun Jan 31 2016 jess.portnoy@borhan.com <Jess Portnoy> - 11.7.0-9
...
* Sun Jan 17 2016 jess.portnoy@borhan.com <Jess Portnoy> - 11.6.0-13
...
* Fri Jan 8 2016 jess.portnoy@borhan.com <Jess Portnoy> - 11.6.0-5
...
* Sun Jan 3 2016 jess.portnoy@borhan.com <Jess Portnoy> - 11.5.0-9
...
* Mon Dec 21 2015 jess.portnoy@borhan.com <Jess Portnoy> - 11.4.0-12
...
* Sat Dec 19 2015 jess.portnoy@borhan.com <Jess Portnoy> - 11.4.0-11
- SUP-6332 - Live entry has not been fully uploaded / transcoded
- SUP-6763 - Cross Borhan connector does not export thumbnails
...
* Sun Dec 6 2015 jess.portnoy@borhan.com <Jess Portnoy> - 11.3.0-12
...
* Sun Nov 22 2015 jess.portnoy@borhan.com <Jess Portnoy> - 11.2.0-12
- 11.2.0 final

* Wed Nov 18 2015 jess.portnoy@borhan.com <Jess Portnoy> - 11.2.0-8
...
* Mon Nov 9 2015 jess.portnoy@borhan.com <Jess Portnoy> - 11.1.0-11
- Final 11.1.0 release

* Fri Nov 6 2015 jess.portnoy@borhan.com <Jess Portnoy> - 11.1.0-10
- SUP-5976 - Channel - kuser_id is '0' Causing Redundant Errors Sent to Users
- SUP-4924 - API changes 'BorhanFlavorAssetFilter'
...
* Sun Oct 25 2015 jess.portnoy@borhan.com <Jess Portnoy> - 11.0.0-12
- SUP-6102 - Default Metadata not working via Krecord (BB/ KMS)
- PLAT-3920 - add Like->list

* Wed Oct 21 2015 jess.portnoy@borhan.com <Jess Portnoy> - 11.0.0-7
- Pre create dirs under app/cache in order to set them to proper permissions

* Thu Oct 15 2015 jess.portnoy@borhan.com <Jess Portnoy> - 11.0.0-4
...
* Sun Oct 11 2015 Jess Portnoy <jess.portnoy@borhan.com> - 10.21.0-9
...
* Mon Sep 21 2015 jess.portnoy@borhan.com <Jess Portnoy> - 10.20.0-11
...
* Mon Sep 7 2015 jess.portnoy@borhan.com <Jess Portnoy> - 10.19.0-9
...
- SUP-5243 - BMC strips BorhanEndUserReportInputFilter when generating CSV
...
* Thu Aug 6 2015 jess.portnoy@borhan.com <Jess Portnoy> - 10.17.0-10
...
* Fri Jul 24 2015 Jess Portnoy <jess.portnoy@borhan.com> - 10.16.0-6
...
- PLAT-3294 - OR between BorhanCategoryEntryAdvancedFilter and BorhanMetadataSearchItem produces AND
...
* Sun Jun 14 2015 Jess Portnoy <jess.portnoy@borhan.com> - 10.13.0-5
...
- SUP-4001 - Category listing options is not filtering on BMC
...
* Sun May 31 2015 Jess Portnoy <jess.portnoy@borhan.com> - 10.12.0-5
...
* Sun May 17 2015 Jess Portnoy <jess.portnoy@borhan.com> - 10.11.0-7
...
- SUP-3451 - BMC error - The language 'sv_SE' has to be added before it can be used. 
...
- PLAT-2885 - Borhan Live with DVR: Player stuck for a few minutes after loading page
...
* Sun Apr 26 2015 Jess Portnoy <jess.portnoy@borhan.com> - 10.9.0-7
- SUP-4081 - distributing from remote storage
- PLAT-2055 - Trim is not performed on Borhan Live recorded VOD entry.
...
* Mon Apr 13 2015 Jess Portnoy <jess.portnoy@borhan.com> - 10.9.0-5
...
* Sun Apr 5 2015 Jess Portnoy <jess.portnoy@borhan.com> - 10.8.0-12
- Run installPlugins.php post upgrade.

* Sun Apr 5 2015 Jess Portnoy <jess.portnoy@borhan.com> - 10.8.0-11
- SUP-3451 - BMC error - The language 'sv_SE' has to be added before it can be used.
...
- PLAT-1680 - Server BorhanMediaEntryFilterForPlaylist call does not respect filter limit or pagging
...
* Mon Mar 23 2015 Jess Portnoy <jess.portnoy@borhan.com> - 10.8.0-2
...
* Sun Mar 22 2015 Jess Portnoy <jess.portnoy@borhan.com> - 10.7.0-2
...
* Mon Mar 8 2015 Jess Portnoy <jess.portnoy@borhan.com> - 10.6.0-2
- SUP-3864 - Download gets cut for large flavors
...
* Tue Feb 24 2015 Jess Portnoy <jess.portnoy@borhan.com> - 10.5.0-6
- Fix clipapp ver

* Sun Feb 22 2015 Jess Portnoy <jess.portnoy@borhan.com> - 10.5.0-5
- SUP-2942 - Audio-Video Sync Issue on Borhan Live stream
...
- SUP-3608 - Borhan java client fail on test
...
* Thu Jan 25 2015 Jess Portnoy <jess.portnoy@borhan.com> - 10.3.0-3
...
* Thu Jan 15 2015 Jess Portnoy <jess.portnoy@borhan.com> - 10.3.0-2
- Added the VOD delivery profile.

* Tue Jan 13 2015 Jess Portnoy <jess.portnoy@borhan.com> - 10.3.0-1
- Ver Bounce to 10.3.0

* Sun Jan 10 2015 Jess Portnoy <jess.portnoy@borhan.com> - 10.2.0-5
...
* Sun Dec 28 2014 Jess Portnoy <jess.portnoy@borhan.com> - 10.1.0-2
- Bounce BMC ver.
...
- Borhan Media Server supporting Webcasting BE enhancements
- Borhan-Live DVR Window change to 24 hours
...
* Mon Dec 8 2014 Jess Portnoy <jess.portnoy@borhan.com> - 9.19.8-3
...
* Tue Nov 18 2014 Jess Portnoy <jess.portnoy@borhan.com> - 9.19.7-2
- Added app/configurations/sphinx_schema_update which is read by borhan-sphinx-schema-update.sh to determine rather or not to run
...
- SUP-2935 - Remove Borhan from being automatically added to the whitelisted domains
...
- PLAT-1114 - Test Wirecast Encoder with Borhan Live Platform
...
* Tue Sep 30 2014 Jess Portnoy <jess.portnoy@borhan.com> - 9.19.3-4
- http://forum.borhan.org/t/upgrade-from-9-18-x-to-9-19-x-latest-release-as-on-date-caused-ssl-to-break/662

* Mon Sep 29 2014 Jess Portnoy <jess.portnoy@borhan.com> - 9.19.3-3
- Patch for auto embed to work, should be dropped when core merge.

* Sun Sep 21 2014 Jess Portnoy <jess.portnoy@borhan.com> - 9.19.3-2
- PLAT-1649 - Allow the option to disable BMC "forgot password" functionality per partner
- PLAT-1559 - BMC-login to ignore "remember me" checkbox per partner configuration
- PLAT-1558 - Add partner configuration to "disable" the "remember me" functionality in BMC login
- PLAT-1555 - add XSS protection in API to not allow HTML
- PLAT-1554 - Add ability to change the KS expiration for BMC login per partner
- PLAT-1548 - analytics reports downloaded from BMC do not enforce any access-control policies or any other means of authentication
- PLAT-1547 - stored XSS vulnerability in "my user settings" in the BMC
- PLAT-1886 - Support Live to VOD flow with no "down time" where DVR + Recording is enabled.
...
* Mon Sep 1 2014 Jess Portnoy <jess.portnoy@borhan.com> - 9.19.2-3
- Dropping patches that were already merged to Core.

* Fri Aug 23 2014 Jess Portnoy <jess.portnoy@borhan.com> - 9.19.2-1
- SUP-2591 - Application Error in Canvas iFRAME EMBED
- SUP-2581 - User Unable to Set Thumbnails /BMC
...
- PLAT-1104 - BMC Login: The 'login' button is cut for Espan~ol
- PLAT-1105 - BMC Login: Default captions are not shown in the selected language
- PLAT-1106 - BMC Login: Default captions on the login page are editable

* Sun Jun 29 2014 Jess Portnoy <jess.portnoy@borhan.com> - 9.17.0-10
- From now on - will run update.php at each upgrade instead of specific scripts.

* Tue Jun 25 2014 Jess Portnoy <jess.portnoy@borhan.com> - 9.17.0-4
- Core now need the passwd for partner 99. Manually merged from 9.18.0.

* Tue Jun 24 2014 Jess Portnoy <jess.portnoy@borhan.com> - 9.17.0-3
...
- PLAT-1352 - Tvinci integration - extend POC connector for Borhan Connect Demo
...
- SUP-814 - Media counts in Channels do not match Channel preview or BMC counts
...
- SUP-1987 - Instant blocking of user on BMC
- SUP-2059 - Script error in Multiaccount console
- PLAT-1039 - Borhan live entry gets duration 1 hours.
- PLAT-1311 - Wrong borhan live duration when setting two DCs.
- PLAT-1323 - Sphinx optimization - dynamic attributes within the match optimization

* Mon May 19 2014 Jess Portnoy <jess.portnoy@borhan.com> - 9.15.0-7
...
- PLAT-1241 - Borhan live - add support for setting base URL per customer
...
- PLAT-692 - Freezes occurs when playing borhan live entry. (happens in production as well)
...
- PLAT-952 - Production:Borhan live entry that is stream for many hours can't play->media not found


* Tue Apr 8 2014 Jess Portnoy <jess.portnoy@borhan.com> - 9.14.0-6
...
* Mon Apr 6 2014 Jess Portnoy <jess.portnoy@borhan.com> - 9.14.0-4
...
- SUP-1775 - MP4 corrupted source shows as ready in BMC 
...
* Sun Apr 1 2014 Jess Portnoy <jess.portnoy@borhan.com> - 9.13.0-6
- Here is our problem: the ip{hone,ad}new tags are only good for Akamai HLS, on any other serve method, it makes ip{hone,ad} serves not to work.
  If a user DOES wish to use Akamai HLS, we have the borhan-remote-storage-config.sh for them to run.

* Thu Mar 25 2014 Jess Portnoy <jess.portnoy@borhan.com> - 9.13.0-4
- Log update scripts output to file instead of STDOUT

* Tue Mar 25 2014 Jess Portnoy <jess.portnoy@borhan.com> - 9.13.0-3
- if system.ini is available, source it in borhan_base.sh, good when you run stuff like:
  mysql -h$DB1_HOST -p$DB1_PASS

* Tue Mar 25 2014 Jess Portnoy <jess.portnoy@borhan.com> - 9.13.0-2
- Typo in file path.
...
* Tue Mar 20 2014 Jess Portnoy <jess.portnoy@borhan.com> - 9.12.0-7
...
- Added the ability to create transcoding profiles with no flavors based on the permission "Enable BMC transcoding profile selection for draft entries".
...
- SUP-1505 - Is there a limitation for bulk download from BMC? Closed
...
* Mon Feb 24 2014 Jess Portnoy <jess.portnoy@borhan.com> - 9.11.0-9
- Consent strings added.

* Mon Feb 24 2014 Jess Portnoy <jess.portnoy@borhan.com> - 9.11.0-6
- Post install mail templt: added the debug aliases to Tip #4 discussing troubleshooting procs.

* Sun Feb 23 2014 Jess Portnoy <jess.portnoy@borhan.com> - 9.11.0-4
...
- Fixes:
  ...
  SUP-1382 - BMC Return 2 Source flavors
  PLAT-925 - Bug for PLAT-722 (Moderation with Live): Basic functionality not working consistently Closed

* Sat Feb 15 2014 Jess Portnoy <jess.portnoy@borhan.com> - 9.9.0-48
- fixed typo sphinx.populate.template.rc

* Sat Feb 15 2014 Jess Portnoy <jess.portnoy@borhan.com> - 9.9.0-47
- placeholder @ADMIN_CONSOLE_ADMIN_MAIL@ for mail in monit.conf.

* Sat Feb 15 2014 Jess Portnoy <jess.portnoy@borhan.com> - 9.9.0-46
- monit config moved to %%prefix/app/configurations/monit

* Sat Feb 15 2014 Jess Portnoy <jess.portnoy@borhan.com> - 9.9.0-45
- Integrated David Bezemer's Admin Console changes for monit.

* Sat Feb 15 2014 Jess Portnoy <jess.portnoy@borhan.com> - 9.9.0-44
- Added patched monit temlpates.

* Sat Feb 15 2014 Jess Portnoy <jess.portnoy@borhan.com> - 9.9.0-43
...
* Mon Feb 10 2014 Jess Portnoy <jess.portnoy@borhan.com> - 9.9.0-34
- Fix editing of p99 template.

* Sun Feb 9 2014 Jess Portnoy <jess.portnoy@borhan.com> - 9.9.0-32
...
* Sun Feb 2 2014 Jess Portnoy <jess.portnoy@borhan.com> - 9.9.0-21
- Only PHP CLI is needed.

* Sat Feb 1 2014 Jess Portnoy <jess.portnoy@borhan.com> - 9.9.0-19
- New BDP3 ver v3.9.7 and hence, new 01.uiConf.99.template.xml. 

* Fri Jan 31 2014 Jess Portnoy <jess.portnoy@borhan.com> - 9.9.0-18
- Fixed template for UI conf generation.

* Wed Jan 28 2014 Jess Portnoy <jess.portnoy@borhan.com> - 9.9.0-10
- debugme.

* Wed Jan 28 2014 Jess Portnoy <jess.portnoy@borhan.com> - 9.9.0-9
- %%{prefix} to be owned by %%{borhan_user}, %%{borhan-user}

* Tue Jan 28 2014 Jess Portnoy <jess.portnoy@borhan.com> - 9.9.0-4
- Depend on mailx.

* Tue Jan 28 2014 Jess Portnoy <jess.portnoy@borhan.com> - 9.9.0-3
- Added emails_en.template.ini.

* Mon Jan 27 2014 Jess Portnoy <jess.portnoy@borhan.com> - 9.9.0-1
- Moving to IX-9.9.0

* Mon Jan 20 2014 Jess Portnoy <jess.portnoy@borhan.com> - 9.7.0-33
- Create additionally needed directories under web.

* Sat Jan 17 2014 Jess Portnoy <jess.portnoy@borhan.com> - 9.7.0-28
- Make sure SELinux is not enabled.

* Fri Jan 17 2014 Jess Portnoy <jess.portnoy@borhan.com> - 9.7.0-27
- Do not create system.ini, post installs create it from template.

* Wed Jan 15 2014 Jess Portnoy <jess.portnoy@borhan.com> - 9.7.0-24
- Added web/tmp web/control and web/dropfolders.

* Sun Jan 14 2014 Jess Portnoy <jess.portnoy@borhan.com> - 9.7.0-23
- We have CLI scripts in PHP that need to run post install of the base package.

* Sun Jan 12 2014 Jess Portnoy <jess.portnoy@borhan.com> - 9.7.0-22
- Change name from .*.template.rc to .*.rc.template so that monit.d/*.rc will work correctly.

* Thu Jan 9 2014 Jess Portnoy <jess.portnoy@borhan.com> - 9.7.0-16
- No need for base.ini, there are no tokens there.

* Thu Jan 9 2014 Jess Portnoy <jess.portnoy@borhan.com> - 9.7.0-15
- Replace tokens in local.ini and base.ini as well.

* Thu Jan 9 2014 Jess Portnoy <jess.portnoy@borhan.com> - 9.7.0-13
- Create all base dirs and make sure they will be removed during un.

* Thu Jan 9 2014 Jess Portnoy <jess.portnoy@borhan.com> - 9.7.0-12
- Added creation of /opt/borhan/tmp dir.
- Postinstall message.
...
* Wed Jan 8 2014 Jess Portnoy <jess.portnoy@borhan.com> - 9.7.0-8
- Do not override system.ini, add to it.

* Mon Jan 6 2014 Jess Portnoy <jess.portnoy@borhan.com> - 9.7.0-7
- Added version file.
- Added dep in borhan-postinst.

* Mon Jan 6 2014 Jess Portnoy <jess.portnoy@borhan.com> - 9.7.0-6
- Depend on borhan-monit and also fix PATH and LD_LIBRARY_PATH to include /opt/borhan/{bin,lib}.

* Mon Jan 6 2014 Jess Portnoy <jess.portnoy@borhan.com> - 9.7.0-5
- Fuck me.

* Mon Jan 6 2014 Jess Portnoy <jess.portnoy@borhan.com> - 9.7.0-4
- Correctly replace tokens!

* Mon Jan 6 2014 Jess Portnoy <jess.portnoy@borhan.com> - 9.7.0-3
- Replace tokens.
