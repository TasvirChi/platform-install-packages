...
Source3: borhan.apache.conf.template 
...
Source21: borhan_batch_upload_falcon.zip
...
sed -i "s@\(^bmc_version\)\s*=.*@\1=%{_bmc_version}@g" $RPM_BUILD_ROOT%{prefix}/app/configurations/base.ini
...
sed -i "s@\(^bdp3_wrapper_version\)\s*=.*@\1=%{bdp3_wrapper_version}@g" $RPM_BUILD_ROOT%{prefix}/app/configurations/base.ini
...
cp %{SOURCE1} $RPM_BUILD_ROOT%{prefix}/app/configurations/apache/borhan.ssl.conf.template
cp %{SOURCE3} $RPM_BUILD_ROOT%{prefix}/app/configurations/apache/borhan.conf.template
...
# we bring another in borhan-batch
...
groupadd -r %{borhan_group} -g7373 2>/dev/null || true
useradd -M -r -u7373 -d %{prefix} -s /bin/bash -c "Borhan server" -g %{borhan_group} %{borhan_user} 2>/dev/null || true
...
chown apache.borhan -R %{prefix}/web/content/entry %{prefix}/web/content/uploads/  %{prefix}/web/tmp/
...
if [ "$1" = 2 ];then
	if [ -r "%{prefix}/app/configurations/local.ini" -a -r "%{prefix}/app/configurations/base.ini" ];then
		...
		chown -R %{borhan_user}.%{apache_user} %{prefix}/app/cache/ %{prefix}/log
		chmod 775 %{prefix}/web/content

		if [ -x %{_sysconfdir}/init.d/httpd ];then
			%{_sysconfdir}/init.d/httpd restart
		fi
		# see https://borhan.atlassian.net/wiki/pages/viewpage.action?spaceKey=QAC&title=QA.Core+Deployment+Instructions%3A+Mar+9%2C+2014
