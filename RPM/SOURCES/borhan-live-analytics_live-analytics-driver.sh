...
SPARK_CORES_MAX=1
SPARK_MEMORY=2g
PID_FILE="/opt/borhan/var/run/$NAME/$NAME.pid"
...
case "$1" in
    start)
        ...
        nohup /opt/spark-1.2.2-bin-hadoop2.4/bin/spark-submit --class com.borhan.Live.MainDriver --master local[2] --driver-class-path "/opt/borhan/lib/*" --conf spark.driver.memory=$SPARK_MEMORY --conf spark.default.parallelism=$SPARK_DEFAULT_PARALLELISM /opt/borhan/lib/$NAME.jar >> /opt/borhan/log/$NAME.log 2>&1 &
        PID="$!"
        retval=$?
